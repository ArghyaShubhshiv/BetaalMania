...
void MenuObject::draw() {
    ...
    const char* modeName = (gameMode == 0) ? "EASY" : (gameMode == 2) ? "HARD" : "MEDIUM";
    float mr = (gameMode == 0) ? 0.2f : (gameMode == 2) ? 1.0f : 0.9f;
    float mg = (gameMode == 0) ? 0.9f : (gameMode == 2) ? 0.2f : 0.85f;
    float mb = (gameMode == 0) ? 0.2f : (gameMode == 2) ? 0.1f : 0.1f;
    char difficultyLabel[32];
    sprintf(difficultyLabel, "DIFFICULTY: %s", modeName);
    ...
    drawButton(3,difficultyLabel,    bx,by+gap*2, bw,bh, mr,mg,mb, hoverItem==3);
    drawButton(4,"QUIT",           bx,by+gap*3, bw,bh, 0.52f,0.10f,0.10f, hoverItem==4);

    glDisable(GL_STENCIL_TEST);
    glStencilFunc(GL_ALWAYS,0,0xFF);

    // ── Help hint ──
    renderStr((float)(W/2)-190,(float)(H-34),
              GLUT_BITMAP_HELVETICA_12,
              0.55f,0.65f,0.75f,
              "Click DIFFICULTY to cycle Easy / Medium / Hard before starting.");
    // ── Controls hint ──
