...
void Bullet::draw() {
    ...
    case BULLET_SHOTGUN: {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            glVertex2f(4 * cosf(a), 4 * sinf(a));
        }
        glEnd();
        break;
    }
    case BULLET_SNIPER: {
        // Long purple/white tracer
        glColor4f(0.8f, 0.2f, 1.0f, 0.5f);
        glBegin(GL_QUADS);
        glVertex2f(-20, -2); glVertex2f(8, -2);
        glVertex2f(8, 2);   glVertex2f(-20, 2);
        glEnd();
        glColor3f(1.0f, 1.0f, 1.0f);
        glBegin(GL_QUADS);
        glVertex2f(-15, -1); glVertex2f(8, -1);
        glVertex2f(8, 1);   glVertex2f(-15, 1);
        glEnd();
        break;
    }
    case BULLET_GRENADE: {
        // Greenish round projectile
        glColor3f(0.2f, 0.4f, 0.1f);
        glBegin(GL_POLYGON);
        for (int i = 0; i < 12; i++) {
            float a = i * 3.14159f / 6.0f;
            glVertex2f(7 * cosf(a), 7 * sinf(a));
        }
        glEnd();
        // Highlight
        ...
        for (int i = 0; i < 8; i++) {
            float a = i * 3.14159f / 4.0f;
            glVertex2f(3 * cosf(a) - 2, 3 * sinf(a) - 2);
        }
        glEnd();
        break;
    }
